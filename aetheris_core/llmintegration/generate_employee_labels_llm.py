...
# === FAISS Vectorized Context ===
FAISS_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "faiss", "employees")
with open(os.path.join(FAISS_DIR, "id_map.pkl"), "rb") as f:
    id_map = pickle.load(f)
with open(os.path.join(FAISS_DIR, "texts.pkl"), "rb") as f:
    all_texts = pickle.load(f)

id_map_lookup = dict(zip([f"Employee:{id}" for id in id_map], all_texts))
...
def build_prompt(record_id, origin, text_block):
    ...
    return f"""
You are a cybersecurity workforce classification engine. Given the following employee profile, classify it across the fields below and return a JSON response with ONLY the fields listed.
...
# === Main runner ===
def main():
    records = Employee.objects.all()[:100]

    for origin in ["Django", "FAISS"]:
        for emp in records:
            record_id = f"Employee:{emp.id}"
...
Business Unit: {emp.business_unit}
Country: {emp.country}
City: {emp.city}
""".strip()
            else:
            ...
            try:
                ...
                debug_file = os.path.join(OUTPUT_DIR, f"debug_employee_{emp.id}_{origin}.txt")
