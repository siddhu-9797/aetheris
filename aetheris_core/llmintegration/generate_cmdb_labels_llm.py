...
# === FAISS Paths ===
FAISS_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "faiss", "cmdb")
with open(os.path.join(FAISS_DIR, "id_map.pkl"), "rb") as f:
    id_map = pickle.load(f)
with open(os.path.join(FAISS_DIR, "texts.pkl"), "rb") as f:
    all_texts = pickle.load(f)

id_map_lookup = dict(zip([f"ConfigurationItem:{id}" for id in id_map], all_texts))
...
def build_prompt(record_id, origin, text_block):
    ...
    return f"""
You are a cybersecurity asset classification engine. Given the following configuration item (CI) details, classify it across the fields below and return a JSON response with ONLY the fields listed.
...
# === Main logic ===
def main():
    records = ConfigurationItem.objects.all()[:100]

    for origin in ["Django", "FAISS"]:
        for ci in records:
            record_id = f"ConfigurationItem:{ci.id}"

            if origin == "Django":
                text_block = f"""
Hostname: {ci.hostname}
Asset Type: {ci.asset_type}
OS: {ci.os} {ci.os_version}
Software: {", ".join(ci.software)}
Software Version: {ci.software_version}
Hardware: {ci.hardware_vendor} {ci.model}
Network Zone: {ci.network_zone}
Connectivity: {ci.connectivity}
Security Software: {ci.security_software}
Business Unit: {ci.business_unit}
Department: {ci.department}
Employee: {ci.employee_email} ({ci.employee_id})
Location: {ci.city}, {ci.country}
Owner: {ci.owner}
Security Posture: {ci.security_posture}
""".strip()
            else:
                text_block = id_map_lookup.get(record_id, f"[FAISS vector not found for {record_id}]")
            ...
            try:
                ...
                debug_file = os.path.join(OUTPUT_DIR, f"debug_cmdb_{ci.id}_{origin}.txt")
